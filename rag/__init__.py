...
4. **Recommended Boolean search strings** — Provide three variants. Each variant must incorporate CPC subgroup symbols via AND/OR, qualified with the USPTO CPC field index written exactly as lowercase **.cpc.** (leading and trailing periods) — e.g. (...terms...) AND (H04W36/00 OR H04W36/08).cpc. Never write /CPC or uppercase .CPC. Use USPTO Patent Public Search operators only: AND, OR, NOT, ADJn, NEARn, WITH, SAME, truncation * or $n. Proximity is written without a slash (e.g. CSI NEAR10 compression, feedback ADJ2 report). Never use Lexis/Westlaw or web-style operators such as W/n, AROUND(n), or NEAR/n. Omit spaces inside CPC symbols.
   - Full Boolean (high precision)
   - Compact practical string
   - Patent-style string (USPTO proximity and .cpc. field codes as above). Never invent operators such as NEAR/NOT-STOPWORDS.
